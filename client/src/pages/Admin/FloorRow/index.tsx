...
const FloorRow: React.FC<PropsType> = ({
    ...
    return (
        <Grid item xs={ 12 } container spacing={ 2 } key={ `floor-${ index }` }>
            ...
            <Grid item xs={ 6 }>
                <ProjectImage images={ planningImage ? [planningImage] : null }
                              title={ 'Загрузить планировку' }
                              disabled={ view }
                              handleAddImage={ (event: React.ChangeEvent<any>) =>
                                  handleFloorImageChange(event, id) }
