...
class ProjectItem extends React.Component<PropsType, State> {
	...
	handleStyleChange = (
		...
		this.setState({ ...this.state, style: event.target.value })
	}
	...
	handleWallMaterialChange = (
		...
		this.setState({ ...this.state, wallMaterial: event.target.value })
	}
	...
	handleInsulationChange = (
		...
		this.setState({ ...this.state, insulation: event.target.value })
	}
	...
	handleCeilingChange = (event: SelectChangeEvent) => {
		this.setState({ ...this.state, ceiling: event.target.value })
	}

	handleRoofChange = (
		event: SelectChangeEvent
	) => {
		this.setState({ ...this.state, roof: event.target.value })
	}
					...
					<>
						{projectId && (
							<div className={classes['button-group']}>
								<Button variant='contained' color='primary' onClick={this.handleEditProjectClick}>
									Редактировать
								</Button>
								<Button variant='contained' color='primary' onClick={this.handleDeleteProjectClick}>
									Удалить
								</Button>
							</div>
						)}
...
const mapStateToProps = (state: ProjectsSliceState): StateProps => {
	return {
		projectSaving: getProjectSaving(state)
	}
}

export default connect<StateProps, DispatchProps, {}, ProjectsSliceState>(
	mapStateToProps,
	(dispatch: ThunkDispatch<ProjectsSliceState, void, Action>): DispatchProps => ({
