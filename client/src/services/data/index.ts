	...
	if (isChecked) {
		...
		if (type === FilterType.CHECKBOX) {
			...
			if (existingValue) {
				...
				console.warn(
					"Should not happen. If value was checked it's in the search list"
				)
			}
		}
	}
...
const filterFunctions: Record<string, FilterFunction> = {
	...
	projectPrice: (value, project) =>
		filterRange(value, project, project.projectPrice),
	buildingPrice: (value, project) =>
		filterRange(value, project, project.buildingPrice),
...
const floorFilter = (value: string, project: ProjectDto): boolean => {
	...
	const floors = value.split(',').map(v => {
	...
	return (
		floors.includes(String(project.floors.length)) ||
		(floors.includes('attic') && project.floors.some(fl => fl.isAttic)) ||
		(floors.includes('basement') && project.floors.some(fl => fl.isBasement))
	)
}
...
const filterBedroom = (value: string, pr: ProjectDto): boolean => {
	...
	return (
		bedrooms.includes(String(pr.bedroomCount)) ||
